...
ZIMBRA_BASE_URL = os.getenv("ZIMBRA_BASE_URL", "https://mail.dolma.gov.np")
ZIMBRA_EMAIL = os.getenv("ZIMBRA_EMAIL")
...
# ZIMBRA SOAP BACKEND
# ==========================================
_ZIMBRA_AUTH_TOKEN = None
_ZIMBRA_TOKEN_EXPIRY = 0

def _create_session():
    s = requests.Session()
    s.mount("https://", HTTPAdapter(max_retries=Retry(total=3, backoff_factor=2, status_forcelist=[500, 502, 503, 504])))
    s.verify = False
    return s

def _zimbra_login(email, password, base_url):
    xml = f'<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Body><AuthRequest xmlns="urn:zimbraAccount"><account by="name">{email}</account><password>{password}</password></AuthRequest></soap:Body></soap:Envelope>'
    ...
    token = root.find(".//{urn:zimbraAccount}authToken")
    if token is None or not token.text: raise RuntimeError("Authentication failed.")
    return token.text.strip()

def _validate_token(token, base_url):
    xml = f'<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Header><context xmlns="urn:zimbra"><authToken>{token}</authToken></context></soap:Header><soap:Body><GetInfoRequest xmlns="urn:zimbraAccount"/></soap:Body></soap:Envelope>'
    try:
        res = _create_session().post(f"{base_url}/service/soap", data=xml, headers={"Content-Type": "text/xml; charset=utf-8"}, timeout=(20, 60))
        return res.status_code == 200 and "soap:Fault" not in res.text
    except: return False

def _get_valid_token():
    global _ZIMBRA_AUTH_TOKEN, _ZIMBRA_TOKEN_EXPIRY
    now = time.time()
    if _ZIMBRA_AUTH_TOKEN and now < _ZIMBRA_TOKEN_EXPIRY and _validate_token(_ZIMBRA_AUTH_TOKEN, ZIMBRA_BASE_URL):
        return _ZIMBRA_AUTH_TOKEN
    _ZIMBRA_AUTH_TOKEN = _zimbra_login(ZIMBRA_EMAIL, ZIMBRA_PASSWORD, ZIMBRA_BASE_URL)
    _ZIMBRA_TOKEN_EXPIRY = now + 3600
    return _ZIMBRA_AUTH_TOKEN

def _upload_files(token, files, base_url):
    aids, url = [], f"{base_url}/service/upload?fmt=extended,js"
    s = _create_session()
    s.headers.update({"Cookie": f"ZM_AUTH_TOKEN={token}", "User-Agent": "Mozilla/5.0", "Accept": "application/json"})
    for i, f in enumerate(files, 1):
        f.seek(0)
        res = s.post(url, files={"uploadedFile": (f.name, f.getvalue(), f.type or "application/octet-stream")}, timeout=(30, 300))
        res.raise_for_status()
        m = re.search(r'"aid"\s*:\s*"([^"]+)"', res.text)
        if not m: raise RuntimeError(f"Failed to extract AID for {f.name}")
        aids.append(m.group(1))
    return aids

def _send_email(token, to, cc, subject, body, aids, base_url):
    safe = lambda s: s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    cc_xml = "".join([f'<e t="c" a="{e}"/>' for e in cc])
    att_xml = f'<attach aid="{",".join(aids)}"/>' if aids else ""
  ...
  <soap:Body><SendMsgRequest xmlns="urn:zimbraMail"><m><e t="t" a="{to}"/>{cc_xml}<su>{safe(subject)}</su>{att_xml}<mp ct="text/plain"><content>{safe(body)}</content></mp></m></SendMsgRequest></soap:Body></soap:Envelope>'''
    res = _create_session().post(f"{base_url}/service/soap", data=xml, headers={"Content-Type": "text/xml; charset=utf-8"}, timeout=(30, 180))
    res.raise_for_status()
    if "soap:Fault" in res.text:
        if "AUTH_EXPIRED" in res.text: raise RuntimeError("AUTH_EXPIRED")
        raise RuntimeError(f"Zimbra rejected: {res.text[:300]}")

def send_email_via_zimbra(to, cc, files, subject="रोक्का जानकारी"):
    ...
    try:
        yield f"📤 Uploading {len(files)} attachment(s)...", 40
        time.sleep(0.3)
        ...
        time.sleep(0.3)
        _send_email(token, to, cc, subject, signature, aids, ZIMBRA_BASE_URL)
        yield "✅ Email sent successfully!", 100
        time.sleep(0.5)
    except RuntimeError as e:
        if "AUTH_EXPIRED" in str(e):
            global _ZIMBRA_AUTH_TOKEN; _ZIMBRA_AUTH_TOKEN = None
            token = _get_valid_token()
            yield "🔄 Token refreshed, retrying...", 50
            aids = _upload_files(token, files, ZIMBRA_BASE_URL)
            yield "📧 Retrying send...", 90
            _send_email(token, to, cc, subject, signature, aids, ZIMBRA_BASE_URL)
            yield "✅ Email sent successfully!", 100
        else: raise
...
def render_send_email_page(institution_names):
    ...
    if st.session_state.sending_email:
        with st.status("📤 Sending Email...", expanded=True) as status:
            try:
                ...
                for msg, val in send_email_via_zimbra(to, cc, data["uploaded_files"], "रोक्का जानकारी"):
                    status.update(label=msg, state="running")
                    st.progress(val)
                
                # Log on success
                with sqlite3.connect(DB_NAME) as db:
                    log_email(db, data["selected_user"], data["selected_institution"], ", ".join([to]+cc), data["ref_no"], data.get("pam_ref",""), ZIMBRA_EMAIL)
    ...
    if not st.session_state.sending_email and not st.session_state.show_auth_popup:
        ...
        if submitted:
            if not ZIMBRA_EMAIL or not ZIMBRA_PASSWORD:
                st.error("🔑 Zimbra credentials missing. Check `.env` file.")
                st.stop()
            if not inst:
...
def main():
    ...
    with st.sidebar:
        st.title("📂 Navigation")
        st.info("📧 **Office Email**: chabahil@dolma.gov.np")
        ...
        st.divider()
        st.caption(f"🔄 App v1.2.0 | Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
