...
def _send_email(token, to_list, cc_list, subject, body, aids, base_url):
    safe = lambda s: s.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    to_xml = "".join([f'<e t="t" a="{e}"/>' for e in to_list])
    cc_xml = "".join([f'<e t="c" a="{e}"/>' for e in cc_list]) if cc_list else ""
